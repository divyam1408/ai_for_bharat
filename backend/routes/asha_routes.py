...
async def asha_chat(report_id: int, data: ChatMessage, user: dict = Depends(get_current_user)):
    """Send a message in an ASHA-managed chat session."""
    if user["role"] != "asha_worker":
        raise HTTPException(status_code=403, detail="Only ASHA workers can use this endpoint")
...
async def get_asha_profile(user: dict = Depends(get_current_user)):
    ...
    in_review  = sum(1 for r in reports if
                     ...
                     r["status"] in ("under_review", "feedback_requested"))
    waiting    = sum(1 for r in reports if r["status"] == "pending_review" and not r.get("doctor_id"))
    return {
        "name": profile["name"],
        "email": profile["email"],
