...
async def chat_response(
    ...
    try:
        response = bedrock_client.generate(BEDROCK_MODEL, chat_history, message)
        return response
    ...
    # try:
    #     response = client.invoke_openai(messages, BEDROCK_MODEL)
    #     print(response)
    #     return response
...
"""
    ...
    try:
        ...
        text = bedrock_client.generate_diagnosis_report( BEDROCK_MODEL, messages)
        if text.startswith("```"):
            text = text.split("\n", 1)[1]
